...
namespace APIs.Controllers
{
    ...
    public class ElectronicProductWishListController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteElectronicWishlistByProductID(int productId)
        {
            var ElectronicWishlist = await _context.ElectronicsWishlists.FirstOrDefaultAsync(bw => bw.ProductId == productId);

            if (ElectronicWishlist == null)
            {
                return NotFound();
            }

            _context.ElectronicsWishlists.Remove(ElectronicWishlist);
            await _context.SaveChangesAsync();
